...
import type { UpdateLivroDto } from '../../types';

interface Props {
  initialData: UpdateLivroDto;
  onSubmit: (data: UpdateLivroDto) => Promise<void>;
  onClose: () => void;
}

export const EditLivroForm = ({ initialData, onSubmit, onClose }: Props) => {
  const [formData, setFormData] = useState<UpdateLivroDto>(initialData);
