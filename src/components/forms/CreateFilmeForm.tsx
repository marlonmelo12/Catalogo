...
import type { CreateFilmeDto } from '../../types';

interface Props {
  onSubmit: (data: CreateFilmeDto) => Promise<void>;
  onClose: () => void;
}

export const CreateFilmeForm = ({ onSubmit, onClose }: Props) => {
  const [formData, setFormData] = useState<CreateFilmeDto>({ titulo: '', ano: new Date().getFullYear(), diretor: '', urlCapa: '' });
